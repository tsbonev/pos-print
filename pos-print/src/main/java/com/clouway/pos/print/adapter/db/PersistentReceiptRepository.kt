...
class PersistentReceiptRepository @Inject constructor(private val database: Provider<MongoDatabase>,
                                                      ...
                                                      private val queueCollectionName: String = "receipt-queue",
                                                      private val maxDocuments: Long = 1000L,
                                                      private val maxSizeInBytes: Long = 104857600L)
  : ReceiptRepository {
  override fun queue(receipt: Receipt): String {
    ...
    val receiptDoc = adaptReceipt(receipt)
    
    receipts().insertOne(receiptDoc)

    queuedReceipts().insertOne(receiptDoc)
  ...
  private fun queuedReceipts(): MongoCollection<Document> {
    val db = database.get()
    if (!db.listCollectionNames().contains(queueCollectionName)){
      db.createCollection(queueCollectionName, CreateCollectionOptions().capped(true)
        .maxDocuments(maxDocuments).sizeInBytes( maxSizeInBytes))
    }
    return db.getCollection(queueCollectionName)
  }
}