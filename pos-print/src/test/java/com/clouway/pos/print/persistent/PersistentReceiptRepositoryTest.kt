...
class PersistentReceiptRepositoryTest {
  ...
  private val printer = BackgroundReceiptPrinter(repository, 100)
  ...
  internal class BackgroundReceiptPrinter(private val repo: ReceiptRepository,
                                          private val delay: Long) : Runnable {
    override fun run() {
      while (true){
        repo.getByStatus(ReceiptStatus.PRINTING).forEach {
          Thread.sleep(delay)
          repo.finishPrinting(it.receiptId)
        }
      }
    }
  }
